...
app.controller("ngMap1", function ($rootScope, $scope, $http, NgMap) {
  ...
  $scope.dplaceChanged = function () {
    ...
    console.log(
      ...
      $scope.dplace.geometry.location.lng(),
      $scope.dplace,
    );
  ...
  $rootScope.bookRide = function () {
    if (
      typeof $scope.origin != "undefined" ||
      typeof $scope.destination != "undefined"
    ) {
      $http({
        method: "POST",
        url: "/createRide",
        ...
        .success(function (data) {
          ...
          alert(
            data.message +
              "Your ride is created successfully, Redirecting you to the dashboard !",
          );

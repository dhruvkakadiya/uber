...
exports.loginAdmin = function (msg, callback) {
  ...
  Admin.findOne({ where: { email: email, password: password } }).then(
    function (user) {
      if (user) {
        json_responses = requestGen.responseGenerator(200, {
...
exports.showCustomers = function (msg, callback) {
  ...
  Customer.findAll({
    ...
    if (customers.length > 0) {
      ...
      json_responses = requestGen.responseGenerator(404, {
        data: "Sorry no customers found",
      });
    }
    callback(null, json_responses);
  });
...
exports.showCustomersForApproval = function (msg, callback) {
  var json_responses;
  Customer.findAll({ where: { verifyStatus: 0 }, limit: 50 }).then(
    function (customers) {
      if (customers.length > 0) {
        json_responses = requestGen.responseGenerator(200, { data: customers });
      } else {
...
exports.showDriversForApproval = function (msg, callback) {
  var json_responses;
  Driver.findAll({ where: { verifyStatus: 0 }, limit: 50 }).then(
    function (driver) {
      if (driver.length > 0) {
        json_responses = requestGen.responseGenerator(200, { data: driver });
      } else {
...
exports.verifyDrivers = function (msg, callback) {
  ...
  Driver.update({ verifyStatus: true }, { where: { email: email } }).then(
    function (driver) {
      if (driver) {
        Drivers.update(
          { email: email },
          ...
          function (err, drivers) {
            if (drivers) {
              json_responses = requestGen.responseGenerator(200, {
                data: "Driver Verified",
              });
            } else {
              json_responses = requestGen.responseGenerator(500, {
                data: "Driver Not Verified",
              });
            }
            callback(null, json_responses);
          },
...
exports.verifyCustomers = function (msg, callback) {
  ...
  Customer.update({ verifyStatus: true }, { where: { email: email } }).then(
    function (customer) {
      if (customer) {
        Customers.update(
          { email: email },
...
exports.ignoreDrivers = function (msg, callback) {
  ...
  Driver.update({ isIgnored: true }, { where: { email: email } }).then(
    function (driver) {
      if (driver) {
        Drivers.update(
          { email: email },
...
exports.ignoreCustomers = function (msg, callback) {
  ...
  Customer.update({ isIgnored: true }, { where: { email: email } }).then(
    function (customer) {
      if (customer) {
        Customers.update(
          { email: email },
          ...
          function (err, customers) {
            if (customers) {
              json_responses = requestGen.responseGenerator(200, {
                data: "Customer Ignored",
              });
            } else {
              json_responses = requestGen.responseGenerator(500, {
                data: "Customer Not Ignored",
              });
            }
            callback(null, json_responses);
          },
  ...
  Billings.aggregate(
    ...
    function (err, results) {
      if (err) {
        console.error(err);
        json_responses = requestGen.responseGenerator(500, {
          message: "Error occured in revenue",
        });
      } else {

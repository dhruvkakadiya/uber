...
exports.createRide = function (msg, callback) {
  ...
  Customers.findOne({ email: customerId, verifyStatus: true }).then(
    function (err, customer) {
      if (err) {
        json_responses = requestGen.responseGenerator(500, {
          message: "Customer not found or customer isn't approved",
        });
        ...
        if (customer) {
          Drivers.findOne({ email: driverId, isBusy: false }).then(
            function (err, driver) {
              if (err) {
                json_responses = requestGen.responseGenerator(500, {
                  message: "Sorry Driver Not found or busy right now.",
                });
                callback(null, json_responses);
              } else {
                if (driver) {
                  ...
                  json_responses = requestGen.responseGenerator(500, {
                    message: "Driver Not found or busy right now.",
                  });
                  callback(null, json_responses);
                }
              }
          ...
          json_responses = requestGen.responseGenerator(500, {
            message: "Customer not verified or not found.",
          });
          callback(null, json_responses);
        }
      }
...
exports.driverRides = function (msg, callback) {
  ...
  Rides.find({ driverId: driverId }, function (err, docs) {
    ...
    if (docs.length > 0) {
      ...
      //console.log("error");
      json_responses = requestGen.responseGenerator(500, {
        message: "No Ride Found with driver Id",
      });
    }
    callback(null, json_responses);
  });
...
exports.customerRides = function (msg, callback) {
  ...
  Rides.find({ customerId: customerId }, function (err, docs) {
    ...
    if (docs.length > 0) {
      ...
      //console.log("error");
      json_responses = requestGen.responseGenerator(500, {
        message: "No Ride Found with customer Id",
      });
    }
    callback(null, json_responses);
  });
